...
(function (w, d) {
  ...
  /* --------------------------------------------------------------------
     7. FAQ
     -------------------------------------------------------------------- */
  /* 「｜」は改行位置の制御用。日本語が単語の途中で折り返されないよう文節で区切っています */
  var FAQS = [
    { q: '無料個別診断だけでも｜大丈夫ですか？',
      a: ['はい。｜診断だけで｜終了できます。｜入会を前提に｜話を進める場では｜ありません。｜現在の婚活で｜止まっている｜可能性が高い場所と、｜最初に変えることを｜整理します。'] },
    { q: '無料診断後に、｜入会の案内は｜ありますか？',
      a: ['ご相談内容に応じて、｜有料の継続支援を｜ご案内する｜場合があります。｜診断だけで｜終了しても構いません。｜その場で入会を｜決める必要はなく、｜現在のアプリや相談所を｜続ける方がよいと｜考えられる場合は、｜その旨をお伝えします。'] },
    { q: '事前に準備するものは｜ありますか？',
      a: ['特にありません。｜プロフィール文や｜最近のメッセージが｜ある場合は、｜個人情報を隠して｜見せていただくと、｜より具体的に｜整理できます。'] },
    { q: 'マッチングアプリを｜利用していても｜相談できますか？',
      a: ['はい。｜アプリ、｜ほかの結婚相談所、｜婚活パーティーなど、｜現在利用している｜方法にかかわらず｜相談できます。'] },
    { q: '交際経験が少なくても｜相談できますか？',
      a: ['はい。｜これまでの経験や、｜今お困りのことを伺い、｜ご自身に合う進め方を｜一緒に考えます。｜何から話せばよいか｜分からない状態でも｜大丈夫です。'] },
    { q: '原因を必ず｜特定できますか？',
      a: ['お相手本人の｜考えを断定することは｜できません。｜これまでの活動状況や｜実際のやり取りを｜もとに、｜止まっている｜可能性が高い場所と、｜次に検証する｜改善策を整理します。'] },
    { q: '無料個別診断では、｜どこまで相談できますか？',
      a: ['現在の活動を振り返り、｜見直すポイントと、｜優先して試す具体策を｜ご提案します。｜模擬お見合いや｜全33項目の評価は、｜継続支援の内容と｜なります。'] },
    { q: '遠方からでも｜受けられますか？',
      a: ['はい。｜オンラインで｜全国に対応しています。｜所要時間は｜約60分です。'] }
  ];
  ...
  if (faqRoot) {
    ...
    FAQS.forEach(function (item, i) {
      ...
      html += '<li>' +
        ...
        '<span class="faq__qt">' +
          item.q.split('｜').map(function (c) { return '<span>' + c + '</span>'; }).join('') +
        '</span><span class="ic" aria-hidden="true"></span></button></h3>' +
        '<div class="faq__a" id="' + aid + '" role="region" aria-labelledby="' + qid + '" hidden>' +
        item.a.map(function (p) {
          return '<p class="pwt">' + p.split('｜').map(function (c) { return '<span>' + c + '</span>'; }).join('') + '</p>';
        }).join('') +
        '</div></li>';
    });
    ...
    faqRoot.addEventListener('click', function (ev) {
      ...
      if (!open) {
        track('faq_open', { faq_question: (btn.textContent || '').replace(/｜/g, '').trim().slice(0, 80) });
      }
    });
  }
