...
- 診断説明セクションへ写真 `assets/images/diagnosis.webp` を追加（同セクションのHTML製レポート図版は削除）
- 担当者セクションの見出しを「こんな人が設計しました」へ変更し、**氏名を非表示**に変更
- 「当日に起きることを、先にお伝えします」セクション（営業不安の解消4点）を削除
- FAQ設問の折り返しを文節単位に制御し、単語の途中で改行される問題を修正
- 背景の交互パターンを調整（予約セクションを淡色に変更し、視覚的に強調）
- FAQ以外の本文文字数：2,455字（削除により当初の2,500〜3,500字をわずかに下回っています）

### 要確認（重要）

`index.html` に「診断レポートにまとめてお渡しします」という記述が3箇所あります。

| 箇所 | 記述 |
|---|---|
| ファーストビュー CTA下 | 整理した内容は、診断レポートにまとめてお渡しします |
| 当日の流れ 最後の20分 | 整理した内容は診断レポートにまとめてお渡しします |
| 診断説明セクションの写真 | 画像内のタブレットに「DELIVERABLE 婚活ボトルネック診断レポート」の表示 |

**無料個別診断で実際にレポートを渡す運用でない場合、この3箇所は削除が必要です。**
当初の要件（◆5・◆36）で「実際には渡さないレポートを特典にする」ことは禁止事項とされています。
渡す運用であれば、記載のまま問題ありません。
